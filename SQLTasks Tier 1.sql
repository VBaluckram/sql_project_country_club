...
the member name. */
SELECT DISTINCT name AS fa_name, CONCAT(firstname, surname) AS mem_name
FROM Bookings AS b
...
SELECT subquery.facility, subquery.member,
CASE WHEN subquery.memid = 0 THEN subquery.guestcost * subquery.slots
	 ELSE subquery.membercost * subquery.slots END AS cost
FROM  (SELECT name AS facility, CONCAT(firstname,surname) AS member, Bookings.memid, Bookings.facid, guestcost, membercost, slots, starttime
       FROM Facilities 
       INNER JOIN Bookings ON Bookings.facid = Facilities.facid
       INNER JOIN Members ON Bookings.memid = Members.memid) AS subquery
WHERE subquery.starttime BETWEEN '2012-09-14' AND '2012-09-14 23:59:59'
HAVING cost > 30
ORDER BY cost DESC
...
SELECT name AS facility, COUNT(Bookings.bookid)
        FROM Bookings
...
SELECT name AS facility, strftime('%m', starttime) as month, COUNT(Bookings.bookid)
        FROM Bookings
        INNER JOIN Facilities ON Bookings.facid = Facilities.facid
        WHERE Bookings.memid NOT IN (0)
